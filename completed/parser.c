...
void compileStatement(void) {
  ...
  switch (lookAhead->tokenType) {
  case TK_IDENT:
    varType[count_indent-1] = compileLValue();
    while(lookAhead->tokenType==SB_COMMA){
      eat(SB_COMMA);
      varType[count_indent] = compileLValue();
      count_indent+=1;
    }
    eat(SB_ASSIGN);
    for(int i=0;i<count_indent-1;i++){      
      expType[i]=compileExpression();
      eat(SB_COMMA);
      if(varType[i]->typeClass==TP_INT && expType[i]->typeClass==TP_DOUBLE)
      {}
      else checkTypeEquality(varType[i],expType[i]);
    }
    expType[count_indent-1]=compileExpression();
    break;
  case KW_SUM:
    eat(KW_SUM);

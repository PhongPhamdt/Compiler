...
void compileStatement(void) {
  ...
  switch (lookAhead->tokenType) {
  case TK_IDENT:
    varType[count_ident-1] = compileLValue2();
    while(lookAhead->tokenType==SB_COMMA){
      eat(SB_COMMA);
      varType[count_ident] = compileLValue2();
      count_ident+=1;
      // printf("count_indent::: %d\n",count_ident);
    }
    eat(SB_ASSIGN);
    int i=0;
    // assert("Da an SB_ASSIGN");
       
    expType[i]=compileExpression();
    checkTypeIgnoreIntChar(varType[i],expType[i]);
    ++i;
    while(lookAhead->tokenType == SB_COMMA) {
      eat(SB_COMMA);
      expType[i] = compileExpression();
    ...
    if(i != count_ident)
      error(ERR_INVALID_SYNTAX,lookAhead->lineNo,lookAhead->colNo);
    break;
  case KW_CALL:
    compileCallSt();
    break;
...
Type* compileLValue2(void) {
  ...
  case OBJ_FUNCTION:
    ...
    break;
  case OBJ_CONSTANT:
    error(ERR_INVALID_SYNTAX,currentToken->lineNo,currentToken->colNo);
  default: 
    error(ERR_INVALID_LVALUE,currentToken->lineNo, currentToken->colNo);
  }
...
void compileAssignSt(void) {
  ...
  varType = compileLValue();
    ...
    if (lookAhead->tokenType==KW_IF){
      ...
      compileCondition();
      eat(KW_RETURN);
      expType_1=compileExpression();
      eat(KW_ELSE);
      eat(KW_RETURN);
      expType_2=compileExpression();
      checkTypeEquality(varType, expType_1);
      checkTypeEquality(varType, expType_2);
    }
    else{
      expType_1=compileExpression();
      checkTypeEquality(varType, expType_1);
